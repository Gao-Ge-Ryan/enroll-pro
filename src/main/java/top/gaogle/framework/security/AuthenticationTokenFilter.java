...
public class AuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain chain) throws ServletException, IOException {
        ...
        if (!StringUtils.isEmpty(token) && Boolean.TRUE.equals(Auth0TokenUtil.validateToken(token)) && Boolean.TRUE.equals(Auth0TokenUtil.validateSignTime(GaogleConfig.getProjectSignTime()))
                && cacheUtil.getTokenCacheExist(token)) {
