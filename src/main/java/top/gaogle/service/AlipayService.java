...
import top.gaogle.framework.exception.GetAlipayClientException;
import top.gaogle.framework.i18n.I18nResult;
import top.gaogle.framework.pojo.CertificateDTO;
import top.gaogle.framework.util.*;
import top.gaogle.pojo.dto.RealAlipayPublicKeySignTypeDTO;
...
public class AlipayService extends SuperService {
    ...
    private AlipayClient getPlatformAlipayClient() throws GeneralSecurityException {
        ...
        byte[] saltDecrypt = CertificateUtil.decrypt(certificate.getPrivateKey(), Base64.getDecoder().decode(aliPayConfig.getSalt()));
        String privateKey = new String(CertificateUtil.decryptByPBE(GaogleConfig.getPassAES(), saltDecrypt, Base64.getDecoder().decode(aliPayConfig.getPrivateKey())), StandardCharsets.UTF_8);
        String alipayPublicKey = new String(CertificateUtil.decryptByPBE(GaogleConfig.getPassAES(), saltDecrypt, Base64.getDecoder().decode(aliPayConfig.getAlipayPublicKey())), StandardCharsets.UTF_8);
        return new DefaultAlipayClient(aliPayConfig.getOpenApiDomain(), aliPayConfig.getAppId(), privateKey,
                AlipayConstants.FORMAT_JSON, AlipayConstants.CHARSET_UTF8, alipayPublicKey, aliPayConfig.getSignType());
    }

    private String getRealAlipayPublicKey() throws GeneralSecurityException {
        CertificateDTO certificate = CertificateUtil.loadCertificate(GaogleConfig.getKeystorePath() + File.separator + RegisterConst.ALIPAY_KEYSTORE,
                GaogleConfig.getAlipayStorePass(), RegisterConst.ALIPAY_CERT);
        byte[] saltDecrypt = CertificateUtil.decrypt(certificate.getPrivateKey(), Base64.getDecoder().decode(aliPayConfig.getSalt()));
        return new String(CertificateUtil.decryptByPBE(GaogleConfig.getPassAES(), saltDecrypt, Base64.getDecoder().decode(aliPayConfig.getAlipayPublicKey())), StandardCharsets.UTF_8);
    }

    private AlipayClient getAlipayClientOfEnterprise(String enterpriseId) throws GeneralSecurityException {
        AlipaySecretModel alipaySecretModel = enterpriseMapper.queryAlipaySecretByEnterpriseId(enterpriseId);
        if (alipaySecretModel == null) {
            throw new GetAlipayClientException("企业不存在");
        }
        String alipayAppId = alipaySecretModel.getAlipayAppId();
        String alipayPrivateKey = alipaySecretModel.getAlipayPrivateKey();
        String alipayAlipayPublicKey = alipaySecretModel.getAlipayAlipayPublicKey();
        String alipaySignType = alipaySecretModel.getAlipaySignType();
        String alipaySalt = alipaySecretModel.getAlipaySalt();
        if (StringUtils.isAnyEmpty(alipayAppId, alipayPrivateKey, alipayAlipayPublicKey, alipaySignType, alipaySalt)) {
            throw new GetAlipayClientException("缺失必要参数");
        }
        CertificateDTO certificate = CertificateUtil.loadCertificate(GaogleConfig.getKeystorePath() + File.separator + RegisterConst.ALIPAY_KEYSTORE,
                GaogleConfig.getAlipayStorePass(), RegisterConst.ALIPAY_CERT);
        byte[] saltDecrypt = CertificateUtil.decrypt(certificate.getPrivateKey(), Base64.getDecoder().decode(alipaySalt));
        String privateKey = new String(CertificateUtil.decryptByPBE(GaogleConfig.getPassAES(), saltDecrypt, Base64.getDecoder().decode(alipayPrivateKey)), StandardCharsets.UTF_8);
        String alipayPublicKey = new String(CertificateUtil.decryptByPBE(GaogleConfig.getPassAES(), saltDecrypt, Base64.getDecoder().decode(alipayAlipayPublicKey)), StandardCharsets.UTF_8);
        return new DefaultAlipayClient(aliPayConfig.getOpenApiDomain(), alipayAppId, privateKey, AlipayConstants.FORMAT_JSON, AlipayConstants.CHARSET_UTF8, alipayPublicKey, alipaySignType);
    }

    private RealAlipayPublicKeySignTypeDTO getRealAlipayPublicKeyOfEnterprise(String enterpriseId) throws GeneralSecurityException {
        AlipaySecretModel alipaySecretModel = enterpriseMapper.queryAlipaySecretByEnterpriseId(enterpriseId);
        if (alipaySecretModel == null) {
            throw new GetAlipayClientException("企业不存在");
        }
        ...
        CertificateDTO certificate = CertificateUtil.loadCertificate(GaogleConfig.getKeystorePath() + File.separator + RegisterConst.ALIPAY_KEYSTORE,
                GaogleConfig.getAlipayStorePass(), RegisterConst.ALIPAY_CERT);
        byte[] saltDecrypt = CertificateUtil.decrypt(certificate.getPrivateKey(), Base64.getDecoder().decode(alipaySalt));
        return new RealAlipayPublicKeySignTypeDTO(new String(CertificateUtil.decryptByPBE(GaogleConfig.getPassAES(), saltDecrypt, Base64.getDecoder().decode(alipayAlipayPublicKey)), StandardCharsets.UTF_8), alipaySignType);
    }
    ...
    public void returnUrl(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            //获取支付宝公钥
            String aliPayPublicKey = getRealAlipayPublicKey();
            //获取支付宝POST过来反馈信息
     ...
     * @param payModel 请求参数
     */
    public I18nResult<String> payTradeCancel(AliPayFaceToFaceModel payModel) throws GeneralSecurityException {
        I18nResult<String> result = I18nResult.newInstance();
    ...
    public String enterpriseNotifyUrl(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //异步验签：切记alipayPublicKey是支付宝的公钥，请去open.alipay.com对应应用下查看。
            boolean flag = AlipaySignature.rsaCheckV1(params, aliPayPublicKey, "utf-8", aliPayConfig.getSignType());
            if (flag) {
                        ...
                        if (!BillStatusEnum.VALID.equals(enterpriseBillModel.getStatus())) {
                            transactionTemplate.execute(new TransactionCallbackWithoutResult() {
                                ...
                                protected void doInTransactionWithoutResult(@NonNull TransactionStatus status) {
                                    ...
                                    int effectRow = enterpriseBillMapper.updateBalanceAndStatusAddByBillId(enterpriseBillId, amount, BillStatusEnum.VALID,
    ...
    public void enterpriseReturnUrl(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            log.info("企业端支付回调requestParams:{}", JsonUtil.object2Json(requestParams));
            log.info("企业端支付回调params:{}", params);
            //异步验签：切记alipaypublickey是支付宝的公钥，请去open.alipay.com对应应用下查看。
            boolean flag = AlipaySignature.rsaCheckV1(params, aliPayPublicKey, "utf-8", aliPayConfig.getSignType());
            if (flag) {
    ...
    public I18nResult<String> clientPay(AliPayFaceToFaceModel aliPayFaceToFaceModel) {
        ...
        try {
            ...
            if (infoModel == null) {
                return result.failedBadRequest().setMessage("该用户报名信息不存在");
            }
            if (!Objects.equals(infoModel.getApprove(), 2)) {
                return result.failedBadRequest().setMessage("审核未通过不能支付");
            }
            ...
            editParam.setCreateBy(loginUsername);
            editParam.setCreateAt(timeMillis);
            AlipayClient alipayClient = getAlipayClientOfEnterprise(enterpriseId);
            AlipayTradePagePayRequest request = new AlipayTradePagePayRequest();
    ...
    public String clientNotifyUrl(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            RealAlipayPublicKeySignTypeDTO realAlipayPublicKeySignTypeDTO = getRealAlipayPublicKeyOfEnterprise(billModel.getEnterpriseId());
            String aliPayPublicKey = realAlipayPublicKeySignTypeDTO.getRealAlipayPublicKey();
            //异步验签：切记alipayPublicKey是支付宝的公钥，请去open.alipay.com对应应用下查看。
            boolean flag = AlipaySignature.rsaCheckV1(params, aliPayPublicKey, "utf-8", realAlipayPublicKeySignTypeDTO.getSignType());
            if (flag) {
    ...
    public void clientReturnUrl(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String clientFrontendUrl = new String(aliPayConfig.getClientFrontendUrl());
        try {
            ...
            log.info("用户端支付回调requestParams:{}", JsonUtil.object2Json(requestParams));
            log.info("用户端支付回调params:{}", params);
            String outTradeNo = params.get("out_trade_no");
            RegisterBillModel registerBillModel = registerBillMapper.queryOneById(outTradeNo);
            String enterpriseId = registerBillModel.getEnterpriseId();
            //异步验签：切记alipaypublickey是支付宝的公钥，请去open.alipay.com对应应用下查看。
            //获取支付宝公钥
            RealAlipayPublicKeySignTypeDTO realAlipayPublicKeySignTypeDTO = getRealAlipayPublicKeyOfEnterprise(enterpriseId);
            String aliPayPublicKey = realAlipayPublicKeySignTypeDTO.getRealAlipayPublicKey();
            boolean flag = AlipaySignature.rsaCheckV1(params, aliPayPublicKey, "utf-8", realAlipayPublicKeySignTypeDTO.getSignType());
            if (flag) {
                log.error("clientReturnUrl验签成功");
                String registerPublishId = registerBillModel.getRegisterPublishId();
                String finalUrl = clientFrontendUrl.replace("#{enterpriseId}", enterpriseId)
                        .replace("#{registerPublishId}", registerPublishId);
