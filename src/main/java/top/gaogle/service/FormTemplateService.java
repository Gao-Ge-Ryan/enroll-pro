...
import top.gaogle.dao.master.InterviewTicketTemplateMapper;
import top.gaogle.dao.master.TicketTemplateMapper;
...
public class FormTemplateService extends SuperService {
    ...
    @Autowired
    public FormTemplateService(FormTemplateMapper formTemplateMapper, TicketTemplateMapper ticketTemplateMapper, InterviewTicketTemplateMapper interviewTicketTemplateMapper) {
    ...
    public I18nResult<List<FormTemplateModel>> queryAll() {
        I18nResult<List<FormTemplateModel>> result = I18nResult.newInstance();
        try {
            String enterpriseId = SecurityUtil.getEnterpriseId();
            if (StringUtils.isEmpty(enterpriseId)) {
                return result.failedBadRequest().setMessage("缺失必要参数");
            }
            List<FormTemplateModel> formTemplateModels = formTemplateMapper.queryByEnterpriseId(enterpriseId);
            result.succeed().setData(formTemplateModels);
        } catch (Exception e) {
            log.error("根据id查询表单详情发生异常:", e);
            result.failed().setMessage(I18ResultCode.MESSAGE, "根据id查询表单详情发生异常");
        }
    ...
    public I18nResult<List<TicketTemplateModel>> queryAllByFormTemplateId(FormTemplateFlagEnum flag) {
        I18nResult<List<TicketTemplateModel>> result = I18nResult.newInstance();
        try {
            String enterpriseId = SecurityUtil.getEnterpriseId();
            if (StringUtils.isEmpty(enterpriseId) || flag == null) {
                return result.failedBadRequest().setMessage("缺失必要参数");
            }
            List<TicketTemplateModel> ticketTemplateModels = ticketTemplateMapper.queryAllByFormTemplateFlag(flag, enterpriseId);
            result.succeed().setData(ticketTemplateModels);
        } catch (Exception e) {
            log.error("根据id查询表单详情发生异常:", e);
            result.failed().setMessage(I18ResultCode.MESSAGE, "根据id查询表单详情发生异常");
        }
    ...
    public I18nResult<List<InterviewTicketTemplateModel>> queryAllInterviewByFormTemplateFlag(FormTemplateFlagEnum flag) {
        I18nResult<List<InterviewTicketTemplateModel>> result = I18nResult.newInstance();
        try {
            String enterpriseId = SecurityUtil.getEnterpriseId();
            if (StringUtils.isEmpty(enterpriseId) || flag == null) {
                return result.failedBadRequest().setMessage("缺失必要参数");
            }
            List<InterviewTicketTemplateModel> ticketTemplateModels = interviewTicketTemplateMapper.queryAllByFormTemplateFlag(flag, enterpriseId);
            result.succeed().setData(ticketTemplateModels);
        } catch (Exception e) {
            log.error("根据id查询表单详情发生异常:", e);
            result.failed().setMessage(I18ResultCode.MESSAGE, "根据id查询表单详情发生异常");
