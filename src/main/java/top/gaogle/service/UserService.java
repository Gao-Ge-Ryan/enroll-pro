    ...
    public I18nResult<PageModel<UserModel>> getPageList(UserQueryParam queryParam) {
        ...
        try {
            ...
            if (!StringUtils.isEmpty(roleId)) {
                roleUserIds = userRoleMapper.queryAccountBysByRoleId(roleId);
            }
            queryParam.setUsernames(roleUserIds);
            ...
            if (!CollectionUtils.isEmpty(usernames)) {
                List<RoleModel> roleModels = roleMapper.queryRolesByUsernames(usernames);
                Map<String, List<RoleModel>> usernameRoleGroup = roleModels.stream()
                        .collect(Collectors.groupingBy(RoleModel::getAccountBy));
                for (UserModel userModel : userModels) {
                    userModel.setRoleModels(usernameRoleGroup.getOrDefault(userModel.getUsername(), null));
                }
            }
        ...
        try {
            ...
            transactionTemplate.execute(new TransactionCallbackWithoutResult() {
                ...
                protected void doInTransactionWithoutResult(TransactionStatus status) {
                    userRoleMapper.deleteByAccountBy(accountBy);
