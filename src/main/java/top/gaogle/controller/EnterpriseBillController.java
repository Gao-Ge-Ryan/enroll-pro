...
public class EnterpriseBillController {
     ...
     * 分页查询企业账单信息（企业端）
     */
    @PreAuthorize("hasAuthority(T(top.gaogle.pojo.enums.AuthorityEnum).ENTERPRISE_BILL_VIEW_ENTERPRISE.value())")
    @RateLimiter(time = 60, count = 60, limitType = LimitType.IP)
    @GetMapping("/enterprise_query_page")
    public ResponseEntity<I18nResult<PageModel<EnterpriseBillModel>>> enterpriseQueryByPage(@Querying EnterpriseBillQueryParam queryParam) {
